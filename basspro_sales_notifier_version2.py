...
# In version2 of this application trying to use logic so that if class="sale" is displayed found and
# sku_number=product sku number # both conditions must be met for text messages to be sent
...
# next three commented out lines are used to test a product that is on sale vs a product that is not on sale
...
#    sale_url = "http://www.basspro.com/Remington-Premier-Golden-Saber-Centerfire-Handgun-Ammo/product/82328/"
    driver.get("http://www.basspro.com/Remington-Premier-Golden-Saber-Centerfire-Handgun-Ammo/product/82328/")
    driver.implicitly_wait(30)
    page_title = driver.title
    print(page_title)
    try:
#        element = driver.find_element_by_class_name("sale")
        element_sku_number = driver.find_element_by_class_name("sku number")
        sku_number = element_sku_number
        print(sku_number)
